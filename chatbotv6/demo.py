...
def get_id_list_from(sentence):
    ...
    fo=open('./word2id.txt', 'a', encoding='utf-8')
    #print(sentence)
    #fo.write(sentence)
    #fo.write('---->')
    for str1 in seg_list:
        #fo.write(str1)
        #fo.write('#')
        id = wordToken.word2id(str1)
        if id:
            sentence_id_list.append(wordToken.word2id(str1))
            '''
            fo.write(str1)
            fo.write(':')
            fo.write(str(wordToken.word2id(str1)))
            fo.write(' | ')
            '''
    #fo.write('\n')
    #fo.close()
    ...
    with tf.Session() as sess:
        ...
        while input_seq:
            ...
            if (len(input_id_list)):
                ...
                #print(outputs_seq)
                '''
                if isinstance(outputs_seq, str):
                    seqstring='没话说了。。。'
                else:
                    seqstring=outputs_seq
                '''
